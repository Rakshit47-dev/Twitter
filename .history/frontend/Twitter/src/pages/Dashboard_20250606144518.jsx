...
import CreatePost from "../components/TweetInput";  
...
export default function SimpleTwitterDashboard() {
  ...
  const fetchTweets = async () => {
    try {
      ...
      // Fetch comments for each tweet after fetching tweets
      res.data.forEach((tweet) => {
        fetchComments(tweet.post_id);
      });
    } catch (error) {
  ...
  const fetchComments = async (postId) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/v1/comment/posts/${postId}`
      );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white">
      <div className="flex flex-1">
        <Sidebar onSelect={(tab) => setSelectedTab(tab)} />
