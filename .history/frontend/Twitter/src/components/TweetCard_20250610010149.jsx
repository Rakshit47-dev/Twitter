  ...
  try {
    if (!isLiked) {
      ...
      await axios.post(`http://localhost:3000/api/v1/posts/${postId}/like`, {
        userId: const storedUser = JSON.parse(localStorage.getItem("user"));
  const userId = storedUser?.user_id;, // pass logged-in user's ID
      });
      setLikesCount((prev) => prev + 1);
      setIsLiked(true);
    } else {
      // Unlike the post
      await axios.delete(`http://localhost:3000/api/v1/posts/${postId}/unlike`, {
        data: { userId: currentUserId }, // axios DELETE requires `data` inside config
      });
      setLikesCount((prev) => prev - 1);
      setIsLiked(false);
    }
