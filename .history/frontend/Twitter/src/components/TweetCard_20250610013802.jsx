  ...
  try {
    if (!isLiked) {
      ...
      await axios.delete(`http://localhost:3000/api/v1/likes/${postId}/unlike`, {
        data: { userId: userId }, // axios DELETE requires `data` inside config
      });
      setLikesCount((prev) => prev - 1);
      setIsLiked(false);
    }
...
useEffect(() => {
  const fetchLikes = async (postId, data) => {
    try {
      const res = await axios.get(`http://localhost:3000/api/v1/likes/${postId}/likes`, data);
      setLikesCount(res.data.totalLikes);
      setIsLiked(res.data.likedByUser.includes(userId)); // assuming backend returns list of user_ids
    } catch (err) {
      console.error("Failed to fetch likes:", err.response?.data || err.message);
    }
  };

  fetchLikes(tweet.post_id, {user_id: userId, post_id : tweet.post_id});
