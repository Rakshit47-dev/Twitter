...
import React, { useState } from "react";
import { Trash2, Edit2, Save, X, Heart, HeartBroken } from "lucide-react";
import axios from "axios";
import { motion, AnimatePresence } from "framer-motion";
...
export default function TweetCard({
  ...
  const [editedCommentText, setEditedCommentText] = useState("");
  const [expanded, setExpanded] = useState(false);
  const [liked, setLiked] = useState(tweet.liked_by_user || false);
  ...
  const handleToggleLike = async () => {
    try {
      if (liked) {
        // Unlike
        await axios.post(
          `http://localhost:3000/api/v1/post/${tweet.post_id}/unlike`,
          { user_id: userId }
        );
        setLiked(false);
        ...
        // Like
        await axios.post(
          `http://localhost:3000/api/v1/post/${tweet.post_id}/like`,
          { user_id: userId }
        );
        setLiked(true);
        setLikeCount((count) => count + 1);
            ...
            <>
              ...
              <div className="mt-2 flex items-center space-x-4">
                <button
                  ...
                  aria-controls={`comments-${tweet.post_id}`}
                >
                  💬 Comment
                </button>
                ...
                >
                  {liked ? <Heart size={16} /> : <HeartBroken size={16} />}
                  <span>{likeCount}</span>
                </button>
              </div>
            </>
