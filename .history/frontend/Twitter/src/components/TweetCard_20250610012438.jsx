...
useEffect(() => {
  const fetchLikes = async () => {
    try {
      const res = await axios.get(`http://localhost:3000/api/v1/posts/${postId}/likes`);
      setLikesCount(res.data.totalLikes);
      setIsLiked(res.data.likedByUser.includes(currentUserId)); // assuming backend returns list of user_ids
    } catch (err) {
